...
#Analisis ekspresi gen bertujuan untuk membandingkan tingkat ekspresi 
gen  
#antara dua kondisi biologis (misalnya kanker vs normal)  
#Pada modul ini kita menggunakan pendekatan statistik limma (Linear 
Models 
#for Microarray Data), yang merupakan standar emas untuk data 
microarray.
...
dotplot(ego, showCategory = 10) +
  ggtitle("GO Biological Process Enrichment (Upregulated Genes)")

barplot(ego, showCategory = 10) +
  ggtitle("Top 10 GO Biological Processes")

go_all <- rbind(ego_BP@result, ego_MF@result, ego_CC@result)
library(ggplot2)

go_sig <- go_all[go_all$p.adjust < 0.05,"FoldEnrichment">0 ]

go_BP_sig <- go_sig[go_sig$ONTOLOGY == "BP", ]
go_CC_sig <- go_sig[go_sig$ONTOLOGY == "CC", ]
go_MF_sig <- go_sig[go_sig$ONTOLOGY == "MF", ]

write.csv(up_genes, "upregulated_genes.csv")
write.csv(down_genes, "downregulated_genes.csv")
write.csv(deg_sig,"DEG SIGNIFIKAN.csv")
write.csv(go_results,"GO RESULT")
...
write.csv(top50, "Hasil top50_GSE10072_DEG.csv") \

message("Analisis selesai. File hasil telah disimpan.")
write.table(top50$SYMBOL,
            "gene_list.txt",
            row.names = FALSE,
            col.names = FALSE,
            quote = FALSE)
